...
@Injectable()
export class GangneungCrawlerService {
  constructor(
    private readonly recruitJobRepository: RecruitJobRepository,
    private readonly eventEmitter: EventEmitter2,
  ) {}

  async crawlGangneung(): Promise<void> {
    try {
      const recruitUrl = 'https://www.gn.go.kr/art/cop/bbs/BBSMSTR_000000000071/selectBoardList.do';
      ...
      for (let i = 1; i <= 5; i++) {
        ...
        const title = lists.find(`tr:nth-child(${i})`).find(`div.list_subject`).find('input:nth-child(8)').attr('value')?.trim();
        ...
        if (today === createdMonthAndDate && url && title && title.includes('공개모집')) {
          ...
          const contents = detail$('div.bbs_detail_content').html();
          ...
          if (process.env.ARTINFO_ADMIN_ID && contents) {
            const recruitJob: RecruitJobPayload = {
              ...
              contents: contents,
              companyName: '강릉시립예술단',
              companyImageUrl: 'https://ycuajmirzlqpgzuonzca.supabase.co/storage/v1/object/public/artinfo/system/gangneung_logo.jpg',
              linkUrl: url,
              isActive: true,
            };
      ...
      const logPayload: LogPayload = {
        ...
        className: 'GangneungCrawlerService',
        functionName: 'crawlGangneung',
        message: e.message,
      };
